...
if %errocode%==0 echo The Updater to update Tauser Toll could not be downloaded.
if %errocode%==0 set /p opt=Retry (y/n): 
if %errocode%==0 if %opt%==y goto Setup_Question4_Updater
if %errocode%==0 goto Setup_Question4
set filepath=%windir%\TauserTool\TTUpdater.bat
set orginal_hash=f44dc2f9c44bf39e0fc4d84ca80fa5fe
...
:askforupdate
if %cls%==1 cls
echo Do you want to view the current Changelog and download an update if it looks nice?
echo 1) Download Changelog and Choose option
echo 2) Exit
set /p opt=Option: 
if %opt%==1 goto downloadchangelog
goto home
...
if not exist %windir%\TauserTool\TTChangelog.sys goto downloadchangelog_fail
type NUL>%windir%\TauserTool\temp\empty.sys
fc %windir%\TauserTool\temp\empty.sys %windir%\TauserTool\TTChangelog.sys
if %errorlevel%==0 goto downloadchangelog_fail
type %windir%\TauserTool\Changelog\TTChangelog.sys
set /p opt=Download update (y/n): 
if %opt%==y if exist %windir%\TauserTool\TTUpdater.bat call %windir%\TauserTool\TTUpdater.bat
goto home

:downloadchangelog_fail
if %cls%==1 cls
echo The changelog could not be downloaded, check internet conntection or the servers are down!
pause
goto askforupdate
...
if %updatesetting%==Ask if exist %windir%\TauserTool\wget\wget.exe goto askforupdate
if %updatesetting%==False goto home

:tellnew
if not exist %windir%\TauserTool\Changelog md %windir%\TauserTool\Changelog
if not exist %windir%\TauserTool\Changelog\newChangelog.sys echo 0>%windir%\TauserTool\Changelog\newChangelog.sys
if not exist %windir%\TauserTool\Changelog\TTChangelog.sys goto home
set /p newchangelog=<%windir%\TauserTool\Changelog\newChangelog.sys
if not %newchangelog%==1 goto home
type %windir%\TauserTool\Changelog\TTChangelog.sys
pause
...
echo 9) Stuff used for Tauser Tool
set /p opt=Option:
if %opt%==0 goto home
