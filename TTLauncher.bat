...
set discord=https://discord.gg/JZ8JrgzA8J
...
if not exist %windir%\TauserTool\info md %windir%\TauserTool\info
rem Gets PowerShell Version
for /f "skip=3 tokens=1" %%a in ('powershell -command "$PSVersionTable.PSVersion"') do set Major=%%a
echo Powershell Version: %Major%
echo %Major% > %windir%\TauserTool\info\psversion.sys
rem Gets Architecture
...
echo %osarch% > %windir%\TauserTool\info\architecture.sys
goto %gobackto%
...
rem VBS Fallback ON or OFF
if exist %windir%\TauserTool\TTopener.vbs if "%username%"=="SYSTEM" reg add HKLM\System\Setup /v CmdLine /t REG_SZ /d "cmd.exe /k %windir%\TauserTool\TTopener.vbs" /f
if not exist %windir%\TauserTool\TTopener.vbs if "%username%"=="SYSTEM" reg add HKLM\System\Setup /v CmdLine /t REG_SZ /d "cmd.exe /k %windir%\TauserTool\TTwarning.bat" /f
...
echo.
rem Loading SysInfo
if not exist %windir%\TauserTool\info\architecture.sys set gobackto=afterload
if not exist %windir%\TauserTool\info\architecture.sys goto getSysInfo
if not exist %windir%\TauserTool\info\psversion.sys set gobackto=afterload
if not exist %windir%\TauserTool\info\psversion.sys goto getSysInfo
if exist %windir%\TauserTool\info\architecture.sys set /p osarch=<%windir%\TauserTool\info\architecture.sys
if exist %windir%\TauserTool\info\psversion.sys set /p Major=<%windir%\TauserTool\info\psversion.sys
:afterload
...
rem start /min cmd /c net start wlansvc
net start wlansvc
...
if %cls%==1 cls
echo It would be really useful if we can download stuff via wget.exe (4'000'000 Bytes / 4 MB)
...
if %opt%==3 bitsadmin /transfer "GetwgetTauserTool" /PRIORITY HIGH "https://cdn.discordapp.com/attachments/695294025305030657/1243829749621592156/wget_licence.txt?ex=6652e639&is=665194b9&hm=ffb9324987e155ded94f2f883b2e715c5ba2d8b4a85c04473068b175c0d0e9ed&" %systemdrive%\wget_licence.txt
...
:checkhash_ask
echo Do you want to ignore and continue? (y/n/a (a = again)) 
set /p opt=Opt: 
...
echo.
echo Please Enter the SSID (Name of the WiFi Network).
set /p SSID=SSID: 
...
:write_VBSFallback
rem VBS FallBack
echo Set WshShell = WScript.CreateObject("WScript.Shell") >%windir%\TauserTool\TTopener.vbs
echo Do >>%windir%\TauserTool\TTopener.vbs
echo     WshShell.Run "%windir%\TauserTool\TTwarning.bat", 1, True >>%windir%\TauserTool\TTopener.vbs
echo     WScript.Sleep 1000 >>%windir%\TauserTool\TTopener.vbs
echo Loop >>%windir%\TauserTool\TTopener.vbs
goto %gobackto%
...
set errorcode=%errorlevel%
if %errorcode%==0 echo LAN / WiFi: ON
if %errorcode%==1 echo LAN / WiFi: OFF
...
:rstau
if exist %windir%\TauserTool\TTopener.vbs reg add HKLM\System\Setup /v CmdLine /t REG_SZ /d "cmd.exe /k %windir%\TauserTool\TTopener.vbs" /f
if not exist %windir%\TauserTool\TTopener.vbs reg add HKLM\System\Setup /v CmdLine /t REG_SZ /d "cmd.exe /k %windir%\TauserTool\TTwarning.bat" /f
rem reg add HKLM\System\Setup /v CmdLine /t REG_SZ /d "cmd.exe /k %windir%\TauserTool\TTopener.vbs" /f
...
echo --- List End ---
echo.
echo NOTE: This file opens after your Patch preferences loaded.
echo.
echo 0) Back
echo 1) Add Additional Service (Link)
echo 2) Add Additional Service (Filepath)
echo 3) Remove Additional Service
set /p opt=Option: 
if %opt%==0 goto settings
if %opt%==1 goto settings_additional_add
if %opt%==2 goto settings_additional_file
if %opt%==3 goto settings_additional_rem
...
:settings_additional_file
if %cls%==1 cls
echo --- Add Additional Service ---
echo.
echo NOTE: File will be copied. Enter a .lnk file if the file is to big.
echo.
set /p filepath=Filepath: 
if not exist "%filepath%" (
echo %filepath% doesn't exist! Try again.
pause
)
if not exist "%filepath%" goto settings_additional_file
set /p filename=Filename (and extension): 
copy "%filepath%" "%windir%\TauserTool\additional_services\%filename%"
goto settings_additional
...
if %cls%==1 cls
title Fallback
echo This Fallback Feature opens Tauser Tool again if it gets closed by accident.
echo NOTE: If you close wscirpt.exes while booted in Tauser Tool and the option is on, your System will crash.
echo NOTE: I'm note sure if it will work... In Development...
echo.
if exist %windir%\TauserTool\TTopener.vbs echo Mode: Acitve
if not exist %windir%\TauserTool\TTopener.vbs echo Mode: Inactive
echo.
echo 0) Back
echo 1) Switch Mode
set /p opt=Option: 
...
if not exist %windir%\TauserTool\TTopener.vbs goto settings_fallback_switch_on
if exist %windir%\TauserTool\TTopener.vbs del %windir%\TauserTool\TTopener.vbs /f /q
goto settings_fallback_msg
:settings_fallback_switch_on
set gobackto=settings_fallback_msg
goto write_VBSFallback
:settings_fallback_msg
...
echo Tauser Tool Made by FBW81C
...
rem REDUNDANT Code here, code is tiny bit faster, user has a lot of disk space
...
if %errorlevel%==0 goto store_fail
rem DO NOT CHECK HASH, LINK DOES NOT CONTAIN ANY SPECIFIED VERSION = NO PREDEFINDED HASH
...
set /p opt=Try again (y/n (n = return to home)): 
