...
if not exist %windir%\TauserTool\Changelog\TTChangelog.sys goto downloadchangelog_fail
type NUL>%windir%\TauserTool\temp\empty.sys
...
echo 2) Ask
echo 3) No
echo 4) Check for Updates and Download if one is available
set /p opt=Option: 
...
if %opt%==4 if not exist %windir%\TauserTool\TTUpdater.bat goto Setup_Question4_Updater
if %opt%==4 call %windir%\TauserTool\TTUpdater.bat
